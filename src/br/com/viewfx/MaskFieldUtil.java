...
public abstract class MaskFieldUtil {
    ...
    /**
     * Monta a mascara para os campos de telefone e celular com a possibilidade de numeros com 8 e 9 digitos.
     *
     * @param textField TextField
     */
        ...
        textField.lengthProperty().addListener(new ChangeListener<Number>() {
            ...
            public void changed(ObservableValue<? extends Number> observable, Number oldValue, Number newValue) {
                if (newValue.intValue() < 14) {
                    ...
                    value = value.replaceFirst("(\\d{0})\\/(\\d{2})(\\d{4})(\\d)", "$1($2)$3-$4");
                    textField.setText(value);
                    positionCaret(textField);
                }else{
                    String value = textField.getText();
                    ...
                    value = value.replaceFirst("(\\d{0})\\/(\\d{2})(\\d{5})(\\d)", "$1($2)$3-$4");
                    textField.setText(value);
                    positionCaret(textField);
                }
            }
    ...
    /**
     * Monta a mascara para Moeda, facilitador para conversão p/ Double, aceita até ####.##.
     *
     * @param textField TextField
     */
    public static void monetaryFieldDoubleFacilitator(final TextField textField) {
        //textField.setAlignment(Pos.CENTER_RIGHT);
        textField.lengthProperty().addListener(new ChangeListener<Number>() {
            @Override
            public void changed(ObservableValue<? extends Number> observable, Number oldValue, Number newValue) {
